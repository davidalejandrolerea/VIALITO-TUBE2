...
<hr>
      ...
      <div class="container">
<form action="act1x.php" method="post" id="test">
...
<?php
$ran1=$_GET['ran1'];
//SELECT preguntas.* FROM preguntas WHERE preguntas.id NOT IN(1,4,6) ORDER BY rand() LIMIT 1;
$consulta="SELECT * FROM preguntas WHERE id_examen='$id_examen' AND numero='$ran1'";
if ($resultado=$link->query($consulta)) {
         while ($row=$resultado->fetch_assoc()) {
             ...
             $resp=$row['resp'];
echo "<tr>";
echo "<td colspan='2'><strong>PREGUNTA # 1 :</strong><font class='lead text-primary'> ".$preg."</font><br><br>";

echo "<input type='hidden' name='id_examen' value='".$id_examen."'>";   

echo "<input type='hidden' name='ran1' value='".$ran1."'>"; 

//-----pone aleatorio las respuestas
